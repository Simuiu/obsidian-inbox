...
#### 它会上传我的 Obsidian Vault 吗？

默认不会。这个项目是 local-first 工作流，Vault、正文、字幕、转写和原始材料默认留在本地。

#### 它会绕过平台限制吗？

不会。项目不会尝试绕过登录、验证码、付费墙或私有内容限制。抓不到正文、字幕或可靠转写时，会保留失败状态，不编造总结。

#### 普通总结和深度总结有什么区别？

普通总结用于快速入库和第一遍理解；深度总结用于课程、教学、系统学习、本地视频和明确需要长期复用的内容。

#### 我想支持一个新平台怎么办？

可以提交 Issue，说明平台、示例 URL 或材料类型、是否需要登录、希望抓取什么、期望的 Obsidian 笔记结构。
